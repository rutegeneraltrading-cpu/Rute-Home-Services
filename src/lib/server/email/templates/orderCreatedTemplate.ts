...
import type { OrderCreatedEmailData } from '@/lib/types/email';
...
function buildProductsTitle(items: OrderCreatedEmailData['items']): string {
...
export function orderCreatedTemplate(data: OrderCreatedEmailData): string {
  ...
  return renderNotificationTemplate({
    title: productsTitle,
    notificationMessage:
      'Your order has been placed. Please proceed with payment to confirm your order.',
    sectionTitle: 'Order summary',
    details: [
      { label: 'Order ID', value: data.orderId },
      { label: 'Customer', value: data.customerName },
      { label: 'Number of Items', value: String(data.items.length) },
      { label: 'Total Amount', value: `R${data.total.toFixed(2)}` },
      { label: 'Payment Status', value: data.paymentStatus },
    ],
    customHtml: itemsDetailsHtml,
    ctaLink: data.dashboardUrl,
    ctaText: 'View Order & Pay',
    preheader: 'Your order was created successfully. Please complete payment.',
  });
}
